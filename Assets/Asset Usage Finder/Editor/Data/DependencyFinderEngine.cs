...
{
    public static class DependencyFinderEngine
    {
        #region Private
            ...
            while (sp.Next(enterChildren: true))
            {
                ...
                if (row == null)
                {
                    ...
                    else
                    {
                        ...
                        row.LabelContent.text = path; // path.Replace(AssetsRootPath, string.Empty);
                ...
                row.Properties.Add(new ResultRow.PropertyData
                {
                    ...
                    Content = new GUIContent
                    {
                        ...
                        text = Nicify(sp, sp.serializedObject.targetObject, row.Main),
                        tooltip = string.Format("{2}{0}.{1}", sp.serializedObject.targetObject.GetType().Name, sp.propertyPath,
                            transformPath)
                    }
                });
            }
        ...
        private static IEnumerable<(int state, string crumbs)> Inner(AnimatorStateMachine f, string crumbs)
        {
        ...
        private static Dictionary<int, Dictionary<int, string>> _animCache = new Dictionary<int, Dictionary<int, string>>();

        private static string GetBread(AnimatorController c, AnimatorState state)
        {
        ...
        private static string Nicify(SerializedProperty sp, Object o, Object main)
        {
        ...
        private static string Nice(string path)
        {
            ...
            if (nice.IndexOf(".m_PersistentCalls.m_Calls", StringComparison.Ordinal) > 0)
                nice = nice.Replace(".m_PersistentCalls.m_Calls", string.Empty).Replace(".m_Target", string.Empty);
        ...
        private static HashSet<string> GetDependencies(Object activeObject)
        {
            ...
            try
            {
                for (int i = 0; i < total; i++)
                {
                    ...
                    var assetDependency = cache.Get(assetPath, objectGuids[i]);

                    if (assetPath.Contains(".unity"))
                        continue;

                    if (assetDependency.Contains(targetGuid))
        ...
        public static HashSet<string> GetScenesThatContain(Object activeObject)
        {
            ...
            try
            {
                for (int i = 0; i < total; i++)
                {
                    ...
                    var assetDependency = cache.Get(assetPath, sceneGuids[i]);
                    if (assetDependency.Contains(targetGuid))
                        results.Add(assetPath);

                    //不显示进度条，防止出现错误：GUI Window tried to begin rendering while something else had not finished rendering! Either you have a recursive OnGUI rendering, or the previous OnGUI 
                    // if (!NeedsUpdate) 
                    //     continue;
        ...
        public static IEnumerable<ResultRow> GetDependenciesInScene(SearchTarget target)
        {
            ...
            for (int ii = 0; ii < SceneManager.sceneCount; ii++)
            {
                ...
                var allObjects = currentScene.GetRootGameObjects().SelectMany(g =>
                {
                    if (g != target.Target)
                ...
                try
                {
                    if (target.Nested.TryGet(out var nested))
                    {
                        ...
                        {
                            var searchTarget = new SearchTarget(n, FinderMode.Scene);
                            for (var i = 0; i < total; i++)
                            {
                                ...
                                if (EditorUtility.DisplayCancelableProgressBar("Searching for file usages in current scene..",
                                    target.Target.name, (float)i / total))
                                    break;
                            }
                        }
                    }
        ...
        private static bool FilterComponents(Component c)
        {
            switch (c)
            {
                case Transform _:
        ...
        public static IEnumerable<ResultRow> GetDependenciesInStage(SearchTarget target, UnityEditor.SceneManagement.PrefabStage stage)
        {
                ...
                .SelectMany(g =>
                {
                    if (g != target.Target)
                        return g.GetComponentsInChildren<Component>(true).Where(FilterComponents).OfType<Object>()
                            .Union(AsEnumerable(g as Object));
                    return g.GetComponentsInChildren<Component>(true).Where(FilterComponents).OfType<Object>();
                }).ToArray();
            var total = allObjects.Length;
            for (int i = 0; i < total; i++)
            {
                ...
                if (EditorUtility.DisplayCancelableProgressBar("Searching for file usages in current scene..", target.Target.name,
                    (float)i / total))
                    break;
            }
